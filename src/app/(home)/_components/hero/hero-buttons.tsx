...
import { env } from "@/env"
import { LINKED_IN_URL } from "@/lib/constants"
...
const HeroButtons = () => {
	return (
		<>
			<Link className={cn(buttonVariants(), "group/link relative z-20  bg-secondary px-10 text-2xl font-bold transition-all duration-500 hover:scale-110 hover:bg-transparent max-sm:px-4")} href={LINKED_IN_URL as unknown as UrlObject} target="_blank">
			...
			<Link className={cn(buttonVariants(), "group/link relative z-20 bg-secondary-foreground px-10 text-2xl font-bold transition-all duration-500 hover:scale-110 hover:bg-transparent max-sm:px-4")} href={env.NEXT_PUBLIC_CV_LINK as unknown as UrlObject} target="_blank">
